<div class="mp_wrapper">
  ...
  <div id="mpca_sub_accounts_used" class="mpca-fat-bottom">
    <h4>
      <?php printf(__('%1$s of %2$s Sub Accounts Used', 'memberpress-corporate'), $ca->num_sub_accounts_used(), $ca->num_sub_accounts);?>
    </h4>
  </div>
...
?>
    ...
    <button id="mpca-add-sub-user-btn" class="mpca-fat-bottom" type="button"
      value=""><?php _e('Add Sub Account', 'memberpress-corporate')?></button>
    <?php endif?>
    ...
    <form action="" method="post" id="mpca-add-sub-user-form" class="mpca-hidden">
      ...
      <input value="" type="text" name="userdata[existing_login]" class="mepr_suggest_user"
        placeholder="<?php _e('Begin Typing Name', 'memberpress', 'memberpress-corporate')?>" />
      <?php else: ?>
      <input value="" type="text" name="userdata[existing_login]" />
      <?php endif?>
      <label>
        <span><?php echo '- ';
_e('OR', 'memberpress-corporate');
echo ' -'; ?></span>
      </label>
      ...
      <!-- Added vaxination file input -->
      <label>
        <span><?php _e('Vaxination', 'memberpress-corporate');?></span>
        <input id="" type="file" name="userdata[vaxination]" />
      </label>
  ...
  <div class="mpca-search mpca-fat-bottom">
    <input id="mpca_sub_account_search" type="text"
      placeholder="<?php _e('Search Sub Accounts...', 'memberpress-corporate');?>" value="<?php echo $search; ?>" />
  </div>
...
?>
      <tr id="mpca-sub-accounts-row-<?php echo $sa->ID; ?>"
        ...
        <td><a href="" data-ca="<?php echo $ca->id; ?>" data-sa="<?php echo $sa->ID; ?>"
            class="mpca-remove-sub-account"><?php _e('Remove', 'memberpress-corporate');?></a></td>
  ...
  <div id="mepr-sub-account-paging">
    ...
    <?php if ($prev_page): ?>
    <a
      href="<?php echo "{$account_url}{$delim}action=manage_sub_accounts&ca={$ca->uuid}&currpage={$prev_page}{$sub_account_search}"; ?>">&lt;&lt;
      <?php _ex('Previous Page', 'ui', 'memberpress-corporate');?></a>
    <?php endif;?>
    <?php if ($next_page): ?>
    <a href="<?php echo "{$account_url}{$delim}action=manage_sub_accounts&ca={$ca->uuid}&currpage={$next_page}{$sub_account_search}"; ?>"
      style="float:right;"><?php _ex('Next Page', 'ui', 'memberpress-corporate');?> &gt;&gt;</a>
    <?php endif;?>
  </div>
...
?>
  ...
  <div id="mpca_signup_url" class="mpca-fat-bottom">
    ...
    <p>
      <?php _e('People signing up with this link will be automatically added to your account', 'memberpress-corporate');?>
    </p>
