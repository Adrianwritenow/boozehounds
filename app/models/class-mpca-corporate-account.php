...
class MPCA_Corporate_Account extends MeprBaseModel
{
  ...
  public function num_dog_accounts_used()
  {
    global $wpdb;
    ...
    $q = $wpdb->prepare(
      "SELECT COUNT(*) FROM wp_mepr_dog_accounts WHERE user_id=%s",
      $this->user_id
    );
  ...
  public function remove_sub_account_user($user_id)
  {
    ...
    // Get rid of any sub_account transactions for this corporate account & user
    $q = $wpdb->prepare(
      "
  ...
  public function sub_account_management_url()
  {
    ...
    return $mepr_options->account_page_url('action=manage_dog_accounts&ca=' . $this->uuid);
  }
  ...
  public function dog_account_list_table(
    $order_by = 'name',
    $order = 'ASC'
  ) {
    global $wpdb;
    ...
    $cols = array(
      ...
      'vacc_expiration' => 'd.vacc_expiration',
    );
    ...
    $dog_table = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}mepr_dog_accounts WHERE user_id = $this->user_id AND ca_id = $this->id ORDER BY name");

    return $dog_table;
  }

  public function sub_account_list_table(
    $order_by = 'last_name',
    $order = 'ASC',
    $paged = '',
    $perpage = 10,
